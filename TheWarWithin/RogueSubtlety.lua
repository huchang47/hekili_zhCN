...
spec:RegisterSetting( "dance_one_charge", false, {
    name = strformat( "%s %s: ：为爆发保留1层充能", NewFeature, Hekili:GetSpellLinkWithTexture( spec.abilities.shadow_dance.id ) ),
    desc = strformat( "如果勾选，就算禁用爆发，但只要还剩余1层充能，仍将推荐使用 %s。\n\n"
            .. "如果 |W%s的|w |cFFFFD100切换分类|r 不是|cFF00B4FF默认|r，该选项将被禁用。\n\n"
            .. "|cFFFF0000此选项仅在选择了 %s +2 时生效。|r",
...
spec:RegisterSetting( "symbols_one_charge", true, {
    name = strformat( "%s %s: 为爆发保留充能", NewFeature, Hekili:GetSpellLinkWithTexture( spec.abilities.symbols_of_death.id ) ),
    desc = strformat( "如果勾选，就算禁用爆发，但只要每层死亡感知天赋能保留1层充能，仍将推荐使用 %s。\n\n"
