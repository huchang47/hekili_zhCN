...
spec:RegisterSetting( "prevent_hardcasts", false, {
    name = strformat( "%s 和 %s: 移动时仅瞬发", Hekili:GetSpellLinkWithTexture( spec.abilities.pyroblast.id ),
        Hekili:GetSpellLinkWithTexture( spec.abilities.fireball.id ) ),
    desc = function()
        return strformat( "如果勾选，当你移动时，将不会推荐你使用非瞬发的 %s 和 %s 技能。\n\n"
        .. "如果拥有 %s 天赋，并且你的技能即将在 |W%s|w 到期前就绪，那么不强求瞬发。", Hekili:GetSpellLinkWithTexture( spec.abilities.pyroblast.id ), Hekili:GetSpellLinkWithTexture( spec.abilities.fireball.id ),
