...
spec:RegisterSetting( "cooldowns_ttd", 0, {
    name = "目标残血时保留爆发技能",
    desc = strformat( "如果设置大于0，当你的 |cFFDD0000非首领|r 目标预计将在该秒数内死亡时，将 |cFFDD0000不会|r 推荐使用主要爆发技能。"..
            "此设置适用于：\n\n%s\n%s\n%s\n\n此设置为实验性设置，但可能有助于避免在"..
            "低生命值的小怪身上浪费主要爆发技能。",
...
spec:RegisterSetting( "shiv_settings_header", false, {
    name = strformat( "%s选项", Hekili:GetSpellLinkWithTexture( spec.abilities.shiv.id ) ),
    type = "header",
    width = "full"
} )

spec:RegisterSetting( "shiv_one_charge", true, {
    name = strformat( "%s %s：为爆发保留1层充能", NewFeature, Hekili:GetSpellLinkWithTexture( spec.abilities.shiv.id ) ),
    desc = strformat( "如果勾选，就算禁用爆发，但只要还剩余1层充能，仍将推荐使用 %s。\n\n"
            .. "如果 |W%s的|w |cFFFFD100切换分类|r 不是 |cFF00B4FF默认值|r，此功能将被禁用。\n\n"
            .. "|cFFFF0000此设置仅在点出2层 %s 充能天赋时生效。|r",
...
spec:RegisterSetting( "shiv_tea_macro", false, {
    name = strformat( "%s %s：绑定%s宏", NewFeature, Hekili:GetSpellLinkWithTexture( spec.abilities.shiv.id ), Hekili:GetSpellLinkWithTexture( spec.abilities.thistle_tea.id ) ),
    desc = strformat( "如果勾选，当推荐使用 %s 时，会假设你正在使用（通过宏）%s。\n\n" ..
            "此功能将防止 %s 被主动推荐。",
...
spec:RegisterSetting( "shiv_tea_macro_text", nil, {
    name = strformat( "%s %s 绑定%s宏文本", NewFeature, Hekili:GetSpellLinkWithTexture( spec.abilities.shiv.id ), Hekili:GetSpellLinkWithTexture( spec.abilities.thistle_tea.id ) ),
    desc = strformat( "当|cffffd100%s:和%s宏|r 已启用时，请使用以下宏替换你的 %s 技能，以确保同时使用 %s。",
    ...
    width = "full",
    multiline = true,
    get = function () return "#showtooltip 毒刃\n/cast 毒刃\n/castsequence reset=5 菊花茶, null" end,
    set = function () end,
} )

spec:RegisterSetting( "vanish_settings_header", false, {
    name = strformat( "%s 选项", Hekili:GetSpellLinkWithTexture( 1856 ) ),
    type = "header",
    width = "full"
} )

spec:RegisterSetting( "vanish_one_charge", false, {
    name = strformat( "%s：为爆发保留1层充能", Hekili:GetSpellLinkWithTexture( 1856 ) ),
    desc = strformat( "如果勾选，就算禁用爆发，但只要还剩余1层充能，仍将推荐使用 %s。\n\n"
            .. "如果 |W%s的|w |cFFFFD100切换分类|r 不是|cFF00B4FF默认|r，该选项将被禁用。\n\n"
            .. "|cFFFF0000此选项仅适用于通过天赋激活了 %s充能的情况。|r\n\n"
            .. "如果同时使用|cFFFFD100预留消失充能|r功能，则必须满足两个条件。",
...
spec:RegisterSetting( "vanish_charges_reserved", 0, {
    name = strformat( "预留 %s 充能", Hekili:GetSpellLinkWithTexture( 1856 ) ),
    desc = strformat( "如果设置值大于零，若使用 %s 会使你的（小数形式的）充能数少于该设置值，则不会推荐使用该技能。\n\n"
            .. "如果 |cFFFFD100%s：为爆发保留1层充能 开关|r 被勾选，则两个条件都必须满足。",
...
spec:RegisterSetting( "solo_vanish", true, {
    name = strformat( "单人游戏时允许使用 %s", Hekili:GetSpellLinkWithTexture( 1856 ) ),  -- Vanish
    desc = strformat( "如果勾选，即使孤身作战，也会推荐使用 %s，|cFFFF0000但这可能会脱离战斗|r。", Hekili:GetSpellLinkWithTexture( 1856 ) ),
    type = "toggle",
    width = "full"
} )

spec:RegisterSetting( "allow_shadowmeld", nil, {
    name = strformat( "允许使用 %s", Hekili:GetSpellLinkWithTexture( 58984 ) ),  -- Shadowmeld
